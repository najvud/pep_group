...
Проект публикует один сайт с верхним горизонтальным меню, где каждая вкладка переключает ленту на отдельный канал. Данные обновляются через `GitHub Actions` каждые 5 минут, без собственного сервера.
...
3. Workflow `Sync Telegram Mirror` каждые 5 минут запускает `scripts/sync_channels.mjs`.
...
То есть сайт сам обновляет ленты примерно раз в 5 минут. Нужно учитывать, что `GitHub Actions schedule` не гарантирует идеальную точность и иногда может стартовать с задержкой.
